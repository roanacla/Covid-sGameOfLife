...
//    return lazyPositions(self.size).map{ (self[$0.row, $0.col].isAlive ? "*" : " ") + ($0.col == self.size.cols - 1 ? "\n" : "")}.joined()
    return ""
  }
...
public struct Grid: GridProtocol {
  ...
  public func getNumberOfCellsOf(type: CellState) -> Int {
    return _cells.flatMap{$0}.filter{$0 == type}.count
  }
}
...
public extension Grid {
  static func gliderInitializer(pos: GridPosition) -> CellState {
    switch pos {
